...
$(document).ready(function(){
	...
	$('.skillbar2').each(function(){
		$(this).find('.skill-percentage p').text($(this).attr('data-percent'));
	});
/*
	$('.skillbar2').each(function () {
		let percentage = parseInt($(this).attr('data-percent').replace("%", ""));
		jQuery({ Counter: 0 }).animate({ Counter: percentage }, {
		  duration: 1000,
		  easing: 'swing',
		  step: function () {
			
			
			$(this).find('.skill-percentage p').text(Math.ceil(this.Counter));
		  }
		});
	  });
*/
...
$( window ).scroll(function() {
	if($(window).scrollTop() > ($("header").scrollTop()+$("header").innerHeight())-80){
		$(".menu").addClass("nav-bg-show");
	}
	else{
		$(".menu").removeClass("nav-bg-show");
	}
  });
